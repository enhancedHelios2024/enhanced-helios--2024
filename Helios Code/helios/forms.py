...
class ElectionForm(forms.Form):
  ...
  #make it multi line placeholder
  short_name = forms.SlugField(
      label="Election Short Name",
      max_length=400,
      widget= forms.TextInput(attrs={'placeholder': 'Enter the short name (will be used in the URL)'}),
    )
  name= forms.CharField(
      label="Election Name",
      max_length=400,
      widget= forms.TextInput(attrs={'placeholder': 'Enter the election name', 'label': 'Election Name'}),
    ) 
  ...
  help_email = forms.CharField(
        required=False,
        initial="",
        widget=forms.TextInput(attrs={'placeholder': 'An email address voters should contact if they need help.'})
    )
  ...
  #                                  widget=SplitSelectDateTimeWidget, required=False)
  voting_starts_at = forms.SplitDateTimeField(
        help_text='UTC date and time when voting begins',
  ...
  voting_ends_at = forms.SplitDateTimeField(
